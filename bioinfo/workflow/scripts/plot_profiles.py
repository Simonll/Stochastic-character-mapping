...
def plot_profiles(args_input, args_infer, args_output):
    ...
    if args_input != "":
        ...
        subprocess.call(
            cmd_plot.format(args_input, args_output, name, nperline), shell=True
        )
    ...
    for profile in args_infer:
        prefs = (
            args_output
            + "/"
            + os.path.basename(profile).replace(".siteprofiles", ".prefs")
        )
        ...
        if args_input != "":
            ...
            subprocess.call(
                cmd_diff_plot.format(
                    prefs + ".diff", args_output, name + ".diff", nperline
                ),
                shell=True,
            )

    plot_correlation(
        os.path.join(args_output, "correlation.aa-preferences.png"),
        axis_dict,
        {},
        global_min_max=True,
        alpha=0.0025,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", required=False, default="", type=str, dest="input"
    )
    parser.add_argument(
        "-p", "--infer", required=True, type=str, nargs="+", dest="infer"
    )
